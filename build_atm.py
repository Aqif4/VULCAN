...
class Atm(object):
	...
	def load_TPK(self, data_atm):
		...
		if self.Kzz_prof == 'const':
    			...
    			# Calculate Kzz using the given formula for P <= 0.5 bar
			Kzz = 5.6e4 * ((data_atm.pico[1:-1]*1e-6) ** -0.5)  # P in bar, pico contains pressure in dynes
